...
namespace Chromely.Loader
{
	...
	public static class CefBinariesLoader
	{
		...
		/// <param name="chromiumDownloadUrlBuilder">Builder which is for building download url.</param>
		/// <returns>The list of temporary files generated</returns>
		public static List<string> Load(IChromelyConfiguration config, IChromiumDownloadUrlBuilder chromiumDownloadUrlBuilder)
		{
			try
			{
				...
				catch (Exception ex)
				{
					...
					if (config.CefDownloadOptions.AutoDownloadWhenMissing)
					{
						...
						{
							CefLoader.Download(config, chromiumDownloadUrlBuilder);
							CefLoader.SetMacOSAppName(config);
						}
						else
						{
							var fileList = DownloadAndNotifyUser(config, chromiumDownloadUrlBuilder);
							CefLoader.SetMacOSAppName(config);
							return fileList;
						}
					}
		...
		/// </returns>
		private static List<string> DownloadAndNotifyUser(IChromelyConfiguration configuration, IChromiumDownloadUrlBuilder downloadUrlBuilder)
		{
			...
			try
			{
				...
				// Start downloading chromium.
				CefLoader.Download(configuration, downloadUrlBuilder);
